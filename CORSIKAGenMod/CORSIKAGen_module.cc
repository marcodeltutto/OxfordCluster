  ...
  void CORSIKAGen::openDBs(){
    ...
    for(int i = 0; i < fShowerInputs; i++){
      if(fShowerInputFiles[i].find("*")==std::string::npos){ 
        std::cout << "MARCO: No wildcards are present." << std::endl;
        ...
        std::cout << "MARCO: Wildcards are present." << std::endl;
        //use findMatchingFiles
        std::vector<std::pair<std::string,long>> flist;
	std::string path(gSystem->DirName(fShowerInputFiles[i].c_str()));
        std::cout << "MARCO: Printing path: " << path << std::endl;
	std::string pattern(gSystem->BaseName(fShowerInputFiles[i].c_str()));
        std::cout << "MARCO: Printing pattern: " << pattern << std::endl;

        ::boost::filesystem::recursive_directory_iterator it(path);
        ::boost::filesystem::recursive_directory_iterator endit;

        flist.push_back(std::make_pair("Bullshit", 1)); // First entry is always ignored later

        while(it != endit) {
          //if(::boost::filesystem::is_regular_file(*it) && it->path().extension() == "db") {
          std::cout << "MARCO: Found file with name " << it->path().filename() << std::endl;
          std::string thefile = it->path().filename().string();
          flist.push_back(std::make_pair(path + "/" + thefile, 1));
          //}
          it++;
        }
...
for( boost::filesystem::directory_iterator i( target_path ); i != end_itr; ++i )
{
    ...
    std::cout <<  "MARCO: BETTER PATH: " << i->path().filename() << std::endl;
}
*/
...
for (auto &entry : boost::make_iterator_range(boost::filesystem::directory_iterator(target_path), {})
...
)
{
  ...
  std::cout <<  "MARCO: BETTER PATH: " << path + "/" + entry.path().filename().string() << std::endl;
}
*/
    ...
    for(unsigned int i=0; i<selectedflist.size(); i++){
      ...
      std::cout  << "Fetching: "<<selectedflist[i].first<<" "<<selectedflist[i].second<<"\n";
      //std::string fetchedfile(fIFDH->fetchInput(selectedflist[i].first));
      std::string fetchedfile = selectedflist[i].first; 
      /*LOG_DEBUG("CorsikaGen")*/ std::cout << "CorsikaGen " << "Fetched; local path: "<<fetchedfile;
